...
public class MidiControl {
    ...
    class MidiChannel{
        ...
        public void setInstrument(byte InstrumentNo, MIDImode midiMode){
            if(0 == this.channel){
                MidiControl.this.toastMassage("Channel was not opened.");
                return;
            }
        ...
        public void noteOn(byte pitch, byte velocity){
            if(0 == this.getChannelNo()){
                MidiControl.this.toastMassage("Channel was not initialized.");
                return;
            }
            // MIDIデータの作成
            byte[] buffer = new byte[16];
    ...
    public synchronized boolean Initialize(Context context) {
        ...
        if (null == this.midiManager) {
            ...
            if (context.getPackageManager().hasSystemFeature(PackageManager.FEATURE_MIDI)) {
                ...
                this.toastMassage("Midi was supported.");
            }
            else {
                this.toastMassage("Midi was NOT supported.");
            }
        }
        if(null == this.midiManager){
            return false ;
